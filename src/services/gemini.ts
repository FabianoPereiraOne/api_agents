...
export async function transcribeAudio(audioAsBase64: string, mimeType: string) {
  const response = await gemini.models.generateContent({
    ...
    contents: [
      {
        text: "Transcreva o áudio para português do Brasil. Seja preciso e natural na transcrição. Matenha a pontuação adequada e divida o texto em parágrafos quando pertinente."
      },
      {
        inlineData: {
          mimeType,
...
export async function generateAnswer(
  ...
  - Cite trechos relevantes e profissional
  - Se for cita o contexto, utilize o termo Conteúdo 
  `.trim()
